...
static int team99_kyber_init_sched(struct request_queue *q, struct elevator_type *e)
{
	struct kyber_queue_data *kqd;
...
static void team99_kyber_exit_sched(struct elevator_queue *e)
{
...
static int team99_kyber_init_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
...
static void team99_kyber_exit_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
...
static void team99_kyber_limit_depth(unsigned int op, struct blk_mq_alloc_data *data)
{
	/*
...
static bool team99_kyber_bio_merge(struct blk_mq_hw_ctx *hctx, struct bio *bio,
...
static void team99_kyber_prepare_request(struct request *rq, struct bio *bio)
{
	rq_set_domain_token(rq, -1);
}

static void team99_kyber_insert_requests(struct blk_mq_hw_ctx *hctx,
...
static void team99_kyber_finish_request(struct request *rq)
{
...
static void team99_kyber_completed_request(struct request *rq, u64 now)
{
	struct kyber_queue_data *kqd = rq->q->elevator->elevator_data;
	struct kyber_cpu_latency *cpu_latency;
	unsigned int sched_domain;
...
static struct request *team99_kyber_dispatch_request(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_queue_data *kqd = hctx->queue->elevator->elevator_data;
	
	printk( "In team99_kyber_dispatch_request function\n" );
...
static bool team99_kyber_has_work(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_hctx_data *khd = hctx->sched_data;
	int i;
...
static struct elevator_type kyber_sched = {
	.ops = {
		.init_sched = team99_kyber_init_sched,
		.exit_sched = team99_kyber_exit_sched,
		.init_hctx = team99_kyber_init_hctx,
		.exit_hctx = team99_kyber_exit_hctx,
		.limit_depth = team99_kyber_limit_depth,
		.bio_merge = team99_kyber_bio_merge,
		.prepare_request = team99_kyber_prepare_request,
		.insert_requests = team99_kyber_insert_requests,
		.finish_request = team99_kyber_finish_request,
		.requeue_request = team99_kyber_finish_request,
		.completed_request = team99_kyber_completed_request,
		.dispatch_request = team99_kyber_dispatch_request,
		.has_work = team99_kyber_has_work,
	},
#ifdef CONFIG_BLK_DEBUG_FS
...
MODULE_AUTHOR("team99");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("team99 I/O scheduler");
